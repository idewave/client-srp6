...
class CryptoHash
{
    public:
        ...
        int UpdateData(const uint8_t* data, int len)      { return EVP_DigestUpdate(m_ctx, data, len); }
        int UpdateData(const std::vector<uint8_t>& data)  { return UpdateData(data.data(), data.size()); }
        int UpdateData(const std::string& str)          { return UpdateData(reinterpret_cast<const uint8_t *>(str.data()), str.length()); }

        int Initialize()                                { return EVP_DigestInit(m_ctx, Algorithm()); }
        int Finalize()                                  { return EVP_DigestFinal_ex(m_ctx, m_digest.data(), nullptr); }

        uint8_t* GetDigest()                              { return m_digest.data(); }
        constexpr static inline size_t GetLength()      { return DigestSize; }

    private:
        EVP_MD_CTX* m_ctx;
        std::array<uint8_t, DigestSize> m_digest;
};
