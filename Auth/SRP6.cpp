...
SRP6_SERVER::SRP6_SERVER()
{
    N.SetHexStr("894B645E89E1535BBDAD5B8B290650530801B18EBFBF5E8FAB3C82872A3E9BB7");
    g.SetDword(7);
}

void SRP6_SERVER::CalculateHostPublicEphemeral(void)
{
    b.SetRand(19 * 8);
...
void SRP6_SERVER::CalculateProof(std::string username)
{
    uint8_t hash[20];
...
bool SRP6_SERVER::CalculateSessionKey(uint8_t* lp_A, int l)
{
    A.SetBinary(lp_A, l);
...
bool SRP6_SERVER::CalculateVerifier(std::string& name, std::string& password)
{
    Sha1Hash sha;
    sha.Initialize();
    sha.UpdateData(name);
...
//    salt.SetRand(s_BYTE_SIZE * 8);
    const char* _salt = s.AsHexStr();
    bool ret = CalculateVerifier(reinterpret_cast<const char *>(sha.GetDigest()), _salt);
    OPENSSL_free((void*)_salt);
    return ret;
}

    bool SRP6_SERVER::CalculateVerifier(const char* rI, const char* salt)
    {
        if (s.SetHexStr(salt) == 0 || s.isZero())
            return false;

        BigNumber I;
        I.SetBinary(reinterpret_cast<const uint8_t *>(rI), Sha1Hash::GetLength());

        // in case of leading zeros in the rI hash, restore them
        uint8_t mDigest[Sha1Hash::GetLength()];
...
void SRP6_SERVER::HashSessionKey(void)
{
    uint8_t t[32];
    uint8_t t1[16];
    uint8_t vK[40];
...
bool SRP6_SERVER::Proof(uint8_t* lp_M, int l)
{
...
bool SRP6_SERVER::ProofVerifier(std::string vC)
{
...
void SRP6_SERVER::Finalize(Sha1Hash& sha)
{
    sha.Initialize();
...
bool SRP6_SERVER::SetSalt(const char* new_s)
{
    if (s.SetHexStr(new_s) == 0 || s.isZero())
        return false;
    return true;
}
bool SRP6_SERVER::SetVerifier(const char* new_v)
{
